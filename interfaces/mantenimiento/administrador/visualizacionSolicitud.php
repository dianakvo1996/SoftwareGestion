...
switch ($respuestaSolicitud->getEstado()) {
    case 'R':
        $estado='<label style="background:#61F955;color:#000">Ejecutado</label>';
        break;
    case 'P':
        $estado='<label style="background:#E34321;color:#fff">Pendiente</label>';
        break;
    case 'E':
        $estado='<label style="background:#E34321;color:#3230C4">En Proceso</label>';
        break;
}
...
if ($respuestaSolicitud->getEvidencia()!='') {
	...
	switch($extension){
		...
		break;
		default:
			$ruta2="<a href='principal.php?CONTENIDO=mantenimiento/administrador/visualizacionSolicitud.php&ideSolicitud={$ideSolicitud}#visor'><img src='../EvidenciasCorrectivos/{$respuestaSolicitud->getEvidencia()}' height='200px'></a>";
		break;
	}
